...
function loadImage(img) {
    ...
    img.src = `${baseImageGamePath}${fileName}`;
    console.log(`Image src updated to: ${img.src}`);
}

function updateImageSrc(){
    document.querySelectorAll(".card img").forEach((img) => loadImage(img));
}
updateImageSrc();
...
document.getElementById("arrow-left").addEventListener('click',(e)=>{
    ...
    carousel.style.transform = `translateX(-${Math.max(carousel.dataset.scroll - 1400,minX)}px)`;
...
document.getElementById("arrow-right").addEventListener('click',(e)=>{
    ...
    let minX = 0;
    let maxX = (carousel.querySelectorAll(".card").length * 420)  - 1400;
    carousel.style.transition = `1s ease`
    carousel.style.transform = `translateX(-${Math.min(Number(carousel.dataset.scroll) + 1400,maxX)}px)`;
